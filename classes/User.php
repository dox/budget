...
class User {
	...
	public function getDepartment(): ?string {
		return $this->userData['department'][0] ?? null;
	}

	public function getTitle(): ?string {
		return $this->userData['title'][0] ?? null;
	}

	public function getTelephone(): ?string {
		return $this->userData['telephonenumber'][0] ?? null;
	}

	public function getOffice(): ?string {
		return $this->userData['physicaldeliveryofficename'][0]
			?? $this->userData['roomnumber'][0]
			?? null;
	}

	public function getDn(): ?string {
		return $this->userData['dn'] ?? null;
	}

	public function memberOfCount(): int {
		return count($this->memberOf());
	}

	public function initials(): string {
		$source = trim((string) ($this->getDisplayName() ?? $this->getUsername() ?? 'U'));
		if ($source === '') {
			return 'U';
		}
		...
		return $initials !== '' ? $initials : strtoupper(substr($source, 0, 1));
	}
	...
	public function loggedOnTime() {
		...
		$query = "SELECT TIMESTAMPDIFF(SECOND, MAX(date_created), NOW()) AS seconds_since_last_logon FROM logs WHERE username = ? AND type = 'INFO' AND event LIKE 'User authenticated%'";
	...
	public function availableCostCentres() {
		printArray($this->groups());
		
		$costCentresClass = new CostCentres();
		$costCentres = $costCentresClass->findByOU($this->groups());
